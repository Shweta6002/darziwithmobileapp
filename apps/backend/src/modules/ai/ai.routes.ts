...
aiRoutes.post("/consult", async (req, res, next) => {
  ...
  try {
    ...
    const chatHistory = history.map((h: any) => ({ role: h.role === "user" ? "user" : "model", parts: [{ text: h.text }] }));
...
aiRoutes.post("/analyze-measurements", async (req, res, next) => {
  ...
  if (!measurements) return res.status(400).json({ error: "No measurements provided for AI analysis" });
